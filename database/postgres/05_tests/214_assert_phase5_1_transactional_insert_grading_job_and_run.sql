-- Verifies transactional insert for Phase 5.1 grading_job and grading_run, then rolls back.
...
$$
DECLARE
    suffix text := 'SMOKE_P51_' || txid_current()::text;
...
BEGIN
    ...
    VALUES (
        ...
        suffix || '_EXAM',
        suffix || ' Exam',
        'Phase 5.1 grading runtime transactional smoke',
        'DRAFT',
        v_proctor_user_id
    )
    ...
    IF (SELECT COUNT(*) FROM grading.grading_job WHERE grading_job_id = v_grading_job_id) <> 1 THEN
        RAISE EXCEPTION 'Expected grading.grading_job insert to persist inside transaction scope';
    END IF;

    IF (SELECT COUNT(*) FROM grading.grading_run WHERE grading_run_id = v_grading_run_id) <> 1 THEN
        RAISE EXCEPTION 'Expected grading.grading_run insert to persist inside transaction scope';
    END IF;

    RAISE NOTICE 'PASS: Phase 5.1 grading_job and grading_run transactional insert succeeded.';
END
$$;
