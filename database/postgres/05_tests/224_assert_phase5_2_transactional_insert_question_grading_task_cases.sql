-- Verifies transactional inserts for Phase 5.2 question_grading_task cases, then rolls back.
...
$$
DECLARE
    suffix text := 'SMOKE_P52_' || txid_current()::text;
    ...
    v_generated_question_3_id bigint;
...
BEGIN
    ...
    VALUES (
        ...
        suffix || '_EXAM',
        suffix || ' Exam',
        'Phase 5.2 question_grading_task transactional smoke',
        'DRAFT',
        v_proctor_user_id
    )
    ...
    VALUES (
        ...
        suffix || '_Q3',
        'MISA_TASK',
        'Case C waiting capture question',
        10.00,
        '{"source":"smoke-224"}'::jsonb
    )
    RETURNING generated_exam_question_id INTO v_generated_question_3_id;
    ...
    -- Case C: missing capture readiness task
    INSERT INTO grading.question_grading_task (
        grading_run_id,
        grading_job_id,
        exam_submission_id,
    ...
    RETURNING question_grading_task_id INTO v_task_c_id;

    IF (SELECT COUNT(*) FROM grading.question_grading_task WHERE question_grading_task_id = v_task_a_id) <> 1 THEN
        RAISE EXCEPTION 'Case A insert did not persist in transaction scope';
    END IF;

    IF (SELECT COUNT(*) FROM grading.question_grading_task WHERE question_grading_task_id = v_task_b_id) <> 1 THEN
        RAISE EXCEPTION 'Case B insert did not persist in transaction scope';
    END IF;

    IF (SELECT COUNT(*) FROM grading.question_grading_task WHERE question_grading_task_id = v_task_c_id) <> 1 THEN
        RAISE EXCEPTION 'Case C insert did not persist in transaction scope';
    END IF;

    RAISE NOTICE 'PASS: Phase 5.2 question_grading_task transactional cases A/B/C succeeded.';
END
$$;
