-- Verifies Phase 5A transactional insert graph case 1:
-- sealed form/code submission full-match chain with question_grading_profile and full grading evidence.
...
$$
DECLARE
    suffix text := 'SMOKE_P56_C1_' || txid_current()::text;
    ...
    v_question_grading_profile_id bigint;
...
BEGIN
    ...
    VALUES (
        ...
        suffix || '_EXAM',
        suffix || ' Exam',
        'Phase 5.6 case 1 SQL full-match transactional smoke',
        'DRAFT',
        v_proctor_user_id
    )
    ...
    VALUES (
        ...
        10.00,
        'ACTIVE',
        '{"source":"smoke-261"}'::jsonb
    )
    RETURNING question_grading_profile_id INTO v_question_grading_profile_id;
    ...
    VALUES (
        ...
        suffix || '_Q1',
        'SQL_QUERY',
        'Case 1 sealed SQL answer question',
        10.00,
        '{"source":"smoke-261"}'::jsonb
    )
    ...
    VALUES (
        ...
        v_proctor_user_id,
        suffix || '_worker_1',
        '{"case":"case1","step":"score_created"}'::jsonb
    )
    ...
    IF (
        ...
        WHERE qgp.question_grading_profile_id = v_question_grading_profile_id
          AND qgp.input_source = 'SEALED_TEXT_ANSWER'
          AND qgp.answer_language = 'SQL'
          AND qgp.comparison_method = 'EXACT_RESULT_SET'
    ) <> 1 THEN
        RAISE EXCEPTION 'Case 1 question_grading_profile snapshot does not match expected contract';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION 'Case 1 full insert graph verification failed';
    END IF;

    RAISE NOTICE 'PASS: Phase 5A case 1 transactional full-match insert graph succeeded.';
END
$$;
