-- Verifies Phase 3.1.2 insert graph in a transaction and confirms rollback cleanup.
...
$$
DECLARE
    suffix text := 'SMOKE_P312_' || txid_current()::text;
...
BEGIN
    ...
    VALUES (
        ...
        suffix || '_EXAM',
        suffix || ' Exam',
        'Transactional smoke graph for Phase 3.1.2',
        'DRAFT',
        v_proctor_user_id
    )
    ...
    IF v_binding_id IS NULL OR v_incident_id IS NULL OR v_transfer_id IS NULL OR v_reschedule_id IS NULL THEN
        RAISE EXCEPTION 'Phase 3.1.2 insert graph did not create all required rows';
    END IF;

    RAISE NOTICE 'PASS: Phase 3.1.2 transactional insert graph succeeded before rollback.';
END
$$;
...
BEGIN
    IF EXISTS (
        ...
        WHERE hostname LIKE 'smoke_p312_%'
    ) THEN
        RAISE EXCEPTION 'Rollback failed: found persisted exam_session_device_binding rows';
    END IF;

    IF EXISTS (
        SELECT 1
        FROM delivery.exam_session_incident
        WHERE description LIKE 'SMOKE_P312_% Incident'
    ) THEN
        RAISE EXCEPTION 'Rollback failed: found persisted exam_session_incident rows';
    END IF;

    IF EXISTS (
        SELECT 1
        FROM delivery.exam_session_transfer
        WHERE note LIKE 'SMOKE_P312_% Transfer'
    ) THEN
        RAISE EXCEPTION 'Rollback failed: found persisted exam_session_transfer rows';
    END IF;

    IF EXISTS (
        SELECT 1
        FROM delivery.exam_reschedule
        WHERE note LIKE 'SMOKE_P312_% Reschedule'
    ) THEN
        RAISE EXCEPTION 'Rollback failed: found persisted exam_reschedule rows';
    END IF;

    RAISE NOTICE 'PASS: Phase 3.1.2 transactional graph rolled back cleanly.';
END
$$;
