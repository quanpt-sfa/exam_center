-- Verifies transactional inserts for Phase 5.4 question_score and submission_score cases, then rolls back.
...
$$
DECLARE
    suffix text := 'SMOKE_P54_' || txid_current()::text;
    ...
    v_question_score_a_id bigint;
    v_question_score_b_id bigint;
    v_submission_score_a_id bigint;
    v_submission_score_b_id bigint;
BEGIN
    ...
    VALUES (
        ...
        suffix || '_EXAM',
        suffix || ' Exam',
        'Phase 5.4 score transactional smoke',
        'DRAFT',
        v_proctor_user_id
    )
    ...
    VALUES (
        ...
        suffix || '_Q1',
        'SQL_QUERY',
        'Case A form/code answer question',
        10.00,
        '{"source":"smoke-243"}'::jsonb
    )
    ...
    VALUES (
        ...
        suffix || '_Q2',
        'MISA_TASK',
        'Case B capture answer question',
        10.00,
        '{"source":"smoke-243"}'::jsonb
    )
    ...
    -- Case A: standard scored question from sealed answer
    ...
    VALUES (
        ...
        v_sql_engine_id,
        false,
        '{"summary":"perfect"}'::jsonb,
        '{"case":"A"}'::jsonb
    )
    RETURNING question_score_id INTO v_question_score_a_id;
    ...
    RETURNING submission_score_id INTO v_submission_score_a_id;

    -- Case B: capture-based task that needs manual review
    INSERT INTO grading.question_grading_task (
        grading_run_id,
        grading_job_id,
        exam_submission_id,
    ...
    VALUES (
        ...
        v_misa_engine_id,
        true,
        '{"summary":"check manually"}'::jsonb,
        '{"case":"B"}'::jsonb
    )
    RETURNING question_score_id INTO v_question_score_b_id;
    ...
    RETURNING submission_score_id INTO v_submission_score_b_id;

    IF (SELECT COUNT(*) FROM grading.question_score WHERE question_score_id = v_question_score_a_id) <> 1 THEN
        RAISE EXCEPTION 'Case A question_score insert did not persist in transaction scope';
    END IF;

    IF (SELECT COUNT(*) FROM grading.question_score WHERE question_score_id = v_question_score_b_id) <> 1 THEN
        RAISE EXCEPTION 'Case B question_score insert did not persist in transaction scope';
    END IF;

    IF (SELECT COUNT(*) FROM grading.submission_score WHERE submission_score_id = v_submission_score_a_id) <> 1 THEN
        RAISE EXCEPTION 'Case A submission_score insert did not persist in transaction scope';
    END IF;

    IF (SELECT COUNT(*) FROM grading.submission_score WHERE submission_score_id = v_submission_score_b_id) <> 1 THEN
        RAISE EXCEPTION 'Case B submission_score insert did not persist in transaction scope';
    END IF;

    RAISE NOTICE 'PASS: Phase 5.4 transactional score cases A/B succeeded.';
END
$$;
