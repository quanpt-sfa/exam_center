-- Transactional graph insert for Phase 3.1.1 session/check-in runtime path.
...
$$
DECLARE
    suffix text := 'SMOKE_P311_' || txid_current()::text;
...
BEGIN
    ...
    VALUES (
        ...
        suffix || '_EXAM',
        suffix || ' Exam',
        'Transactional smoke graph for Phase 3.1.1',
        'DRAFT',
        v_proctor_user_id
    )
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Graph insert failed for delivery.exam_session';
    END IF;

    IF NOT EXISTS (
        SELECT 1
        FROM delivery.exam_checkin_verification
        WHERE checkin_verification_id = v_checkin_id
    ) THEN
        RAISE EXCEPTION 'Graph insert failed for delivery.exam_checkin_verification';
    END IF;

    RAISE NOTICE 'PASS: Phase 3.1.1 insert graph created session % and check-in %', v_session_id, v_checkin_id;
END
$$;
...
BEGIN
    IF EXISTS (
        SELECT 1
        FROM delivery.exam_session
        WHERE session_code LIKE 'SMOKE_P311_%'
    ) THEN
        RAISE EXCEPTION 'Rollback failed: found persisted exam_session test row';
    END IF;

    IF EXISTS (
        SELECT 1
        FROM delivery.exam_checkin_verification
        WHERE note LIKE 'SMOKE_P311_%'
    ) THEN
        RAISE EXCEPTION 'Rollback failed: found persisted exam_checkin_verification test row';
    END IF;

    RAISE NOTICE 'PASS: Phase 3.1.1 transactional insert graph rolled back cleanly.';
END
$$;
