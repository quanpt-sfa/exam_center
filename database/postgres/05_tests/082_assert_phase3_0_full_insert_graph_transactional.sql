-- Verifies full Phase 3.0 insert graph in a transaction and confirms rollback removes all inserted rows.
...
$$
DECLARE
    suffix text := 'SMOKE_P305_' || txid_current()::text;
...
BEGIN
    ...
    VALUES (
        ...
        suffix || '_EXAM',
        suffix || ' Exam',
        'Transactional full Phase 3.0 graph',
        'DRAFT',
        v_proctor_user_id
    )
    ...
    RETURNING exam_sitting_id INTO v_exam_sitting_2_id;
    ...
    RETURNING exam_assignment_id INTO v_exam_assignment_2_id;
    ...
    RAISE NOTICE 'PASS: Full Phase 3.0 transactional insert graph succeeded before rollback.';
END
$$;
...
$$
BEGIN
    IF EXISTS (SELECT 1 FROM assessment.exam WHERE exam_code LIKE 'SMOKE_P305_%') THEN
        RAISE EXCEPTION 'Rollback check failed: found rows in assessment.exam for SMOKE_P305_ prefix';
    END IF;

    IF EXISTS (SELECT 1 FROM identity.person_photo WHERE photo_ref LIKE 'local://smoke_p305_%') THEN
        RAISE EXCEPTION 'Rollback check failed: found rows in identity.person_photo for smoke_p305 prefix';
    END IF;

    IF EXISTS (SELECT 1 FROM facility.device_registration WHERE registration_value LIKE 'smoke_p305_%') THEN
        RAISE EXCEPTION 'Rollback check failed: found rows in facility.device_registration for smoke_p305 prefix';
    END IF;

    IF EXISTS (SELECT 1 FROM delivery.exam_session_incident WHERE description LIKE 'SMOKE_P305_% Incident') THEN
        RAISE EXCEPTION 'Rollback check failed: found rows in delivery.exam_session_incident for SMOKE_P305_ prefix';
    END IF;

    IF EXISTS (SELECT 1 FROM delivery.exam_session_transfer WHERE note LIKE 'SMOKE_P305_% Transfer') THEN
        RAISE EXCEPTION 'Rollback check failed: found rows in delivery.exam_session_transfer for SMOKE_P305_ prefix';
    END IF;

    IF EXISTS (SELECT 1 FROM delivery.exam_reschedule WHERE note LIKE 'SMOKE_P305_% Reschedule') THEN
        RAISE EXCEPTION 'Rollback check failed: found rows in delivery.exam_reschedule for SMOKE_P305_ prefix';
    END IF;

    RAISE NOTICE 'PASS: Transaction rollback removed all SMOKE_P305_* rows.';
END
$$;
